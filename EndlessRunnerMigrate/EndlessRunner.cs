...
namespace EndlessRunnerMigrate
{
    public class EndlessRunner : Game
    {
        ...
        protected override void LoadContent()
        {
            ...
            //Game Music
            inGameMusic = Content.Load<Song>("Sounds/Music/Jackie Chan - Police Story Theme OST (320  kbps)");
            mainMenuMusic = Content.Load<Song>("Sounds/Music/mainMenu");
            gameOverMusic = Content.Load<Song>("Sounds/Music/gameOver");

            //Initalizes main menu music
            MediaPlayer.Play(mainMenuMusic);

            //Sound Effects
            hkPunch = Content.Load<SoundEffect>("Sounds/Sound Effects/hkPunch");
            hkPunchInstance = hkPunch.CreateInstance();
            star = Content.Load<SoundEffect>("Sounds/Sound Effects/star");
            starInstance = star.CreateInstance();

            //Loads the Sprite Fonts
            titleFont = Content.Load<SpriteFont>("Texts/TitleFont");
            gameFont = Content.Load<SpriteFont>("Texts/GameItems");

            //Jackie Chan's Sprites
            jackieChan = Content.Load<Texture2D>("Images/Sprites/jcman");
            jackieChanRoll = Content.Load<Texture2D>("Images/Sprites/JackieChanRoll ");
            jackieChanPosition = new Vector2(380, 423);
            jackieChanDuck = Content.Load<Texture2D>("Images/Sprites/DuckJackie");
            jackieChanDeath = Content.Load<Texture2D>("Images/Sprites/JackieDeath");
            ...
            startButton = Content.Load<Texture2D>("Images/Background/startBtn");
            startBtnLoc = new Rectangle(355, 271, startButton.Width, startButton.Height);
            quitButton = Content.Load<Texture2D>("Images/Background/quitBtn");
            quitBtnLoc = new Rectangle(398, 447, (int)(quitButton.Width * 0.25), (int)(quitButton.Height * 0.25));
            controlsBtn = Content.Load<Texture2D>("Images/Background/controlsBtn");
            ...
            controls = Content.Load<Texture2D>("Images/Background/controls");
            controlsLoc = new Rectangle(10, 10, (int)(controls.Width * 0.25), (int)(controls.Height * 0.25));
            collectableGuide = Content.Load<Texture2D>("Images/Background/collectables");
            collectableGuideLoc = new Rectangle(550, 10, (int)(collectableGuide.Width * 0.25), (int)(collectableGuide.Height * 0.25));
            escapeKey = Content.Load<Texture2D>("Images/Background/escapeKey");
            ...
            bus = Content.Load<Texture2D>("Images/Sprites/bussprite");
            busLoc = new Rectangle(105, 370, bus.Width, bus.Height);

            //Load Star Sprites and Collision Box
            redStar = Content.Load<Texture2D>("Images/Sprites/redStar");
            orangeStar = Content.Load<Texture2D>("Images/Sprites/orangeStar");
            greenStar = Content.Load<Texture2D>("Images/Sprites/greenStar");
            ...
            itemBackDrop = Content.Load<Texture2D>("Images/Background/backDrop");
            ...
            box = Content.Load<Texture2D>("Images/Sprites/Game Boy Advance - Crash Nitro Kart - Crates");
            boxParameter = new Rectangle(screenWidth, 436, box.Width, box.Height);

            //Loads box crash sprite sheet
            boxCrash = Content.Load<Texture2D>("Images/Sprites/CrashCrate");

            //Loads grass obstacle and its location
            lowGrass = Content.Load<Texture2D>("Images/Sprites/platform");
            ...
            gangster = Content.Load<Texture2D>("Images/Sprites/gangster");
            gangsterRec = new Rectangle(screenWidth, 360, (int)(gangster.Width * 0.40), (int)(gangster.Height * 0.40));

            //Loads gangster death picture
            gangsterDie = Content.Load<Texture2D>("Images/Sprites/GangsterDeath");
